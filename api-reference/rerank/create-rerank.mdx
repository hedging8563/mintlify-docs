...
<ParamField body="top_n" type="integer">
  Number of top results to return. Defaults to all documents. Must be at least `1` and no greater than `documents.length`. AI Sonar currently has no governed provider-specific lower hard cap; if a provider publishes one later, it must be added to rerank request-shape truth before being documented or enforced.
</ParamField>
