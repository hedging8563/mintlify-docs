...
The Responses API é a API de conversação com estado mais recente da OpenAI. AI Sonar suporta este formato como um caminho opcional avançado para modelos compatíveis; use `POST /v1/chat/completions` como a rota padrão compatível com OpenAI, a menos que você precise explicitamente do comportamento específico do Responses.

## Corpo da Requisição

<ParamField body="model" type="string" required>
  ID do modelo a ser usado. Veja [Models](https://aisonar.dev/models) para as opções disponíveis.
</ParamField>
...
<ParamField body="tools" type="array">
  ...
  Para ferramentas hospedadas `image_generation` que usam o modelo de ferramenta de imagem padrão ou definem explicitamente `model: "gpt-image-2"`, a AI Sonar remove `input_fidelity` antes de encaminhar a solicitação, porque o GPT Image 2 já trata entradas de imagem como alta fidelidade. Não envie `background: "transparent"` para esta ferramenta; a AI Sonar não o remove silenciosamente porque isso altera a semântica de saída.
</ParamField>
