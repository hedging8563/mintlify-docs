...
<ParamField body="messages" type="array" required>
  ...
  `content`가 배열인 경우, AI Sonar는 호환되는 모델에 대해 구조화된 멀티모달 블록을 지원합니다:
  - text: `{ "type": "text", "text": "..." }`
  - 이미지: `{ "type": "image_url", "image_url": { "url": "https://..." } }`
  - 비디오: `{ "type": "video_url", "video_url": { "url": "https://..." } }`
  - 오디오: `{ "type": "audio_url", "audio_url": { "url": "https://..." } }`

  멀티모달 프로덕션 트래픽의 경우, 공개 `https` URL을 우선 사용하세요. AI Sonar는 이러한 미디어 블록을 라우팅된 물리적 모델에서 요구하는 제공자별 요청 형식으로 변환합니다.
</ParamField>
