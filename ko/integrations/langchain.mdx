---
title: "LangChain"
description: "표준 OpenAI 호환 chat 및 embeddings를 사용하여 AI Sonar를 LangChain과 통합합니다"
---

## 개요

AI Sonar는 표준 OpenAI 호환 chat 및 embeddings 인터페이스를 사용할 때 LangChain의 `ChatOpenAI` 및 `OpenAIEmbeddings` 통합과 잘 동작합니다.
...
<AccordionGroup>
  <Accordion title="`base_url`을 명시적으로 전달">
    가장 안정적인 AI Sonar 설정은 오래된 environment-variable alias에 의존하는 대신 `base_url="https://api.aisonar.dev/v1"`를 `ChatOpenAI`와 `OpenAIEmbeddings`에 직접 전달하는 것입니다.
  </Accordion>
