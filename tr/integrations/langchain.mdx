---
title: "LangChain"
description: "Standart OpenAI uyumlu chat ve embeddings kullanarak AI Sonar'yı LangChain ile entegre edin"
---

## Genel Bakış

AI Sonar, standart OpenAI uyumlu chat ve embeddings yüzeyinde kaldığınızda LangChain'in `ChatOpenAI` ve `OpenAIEmbeddings` entegrasyonlarıyla iyi çalışır.
...
<AccordionGroup>
  <Accordion title="`base_url` değerini açıkça iletin">
    En güvenilir AI Sonar kurulumu, eski environment variable alias'larına bağlı kalmak yerine `base_url="https://api.aisonar.dev/v1"` değerini doğrudan `ChatOpenAI` ve `OpenAIEmbeddings`'e iletmektir.
  </Accordion>
