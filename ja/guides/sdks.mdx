---
title: "SDKとライブラリ"
description: "AI Sonar 連携に推奨される SDK とライブラリ"
---

## 概要

AI Sonar は、必要な動作に合わせて SDK を選ぶことで最も簡単に統合できます:
...
| 既存の OpenAI 互換クライアント | AI Sonar の `/v1` 上の OpenAI SDK |
...
<AccordionGroup>
  ...
  <Accordion title="移行には OpenAI 互換の `/v1` を使用">
    OpenAI 互換 SDK は、既存の OpenAI 形式クライアント、移植性のあるチャット、そして embeddings に最適な経路です。これらは互換経路として扱ってください。AI Sonar の唯一の API 面ではありません。
  </Accordion>

  <Accordion title="Base URL を明示しておく">
    古い環境変数エイリアスに頼るのではなく、AI Sonar のベース URL をクライアント設定に直接指定してください。
  </Accordion>
</AccordionGroup>
