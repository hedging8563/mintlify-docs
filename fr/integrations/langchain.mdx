---
title: "LangChain"
description: "Intégrez AI Sonar à LangChain à l’aide du chat et des embeddings standard compatibles OpenAI"
---

## Vue d’ensemble

AI Sonar fonctionne bien avec les intégrations `ChatOpenAI` et `OpenAIEmbeddings` de LangChain lorsque vous restez sur la surface standard de chat et d’embeddings compatible OpenAI.
...
texts = [
    ...
    "AI Sonar prend en charge les intégrations compatibles OpenAI."
]
...
<AccordionGroup>
  <Accordion title="Transmettre `base_url` explicitement">
    La configuration AI Sonar la plus fiable consiste à transmettre `base_url="https://api.aisonar.dev/v1"` directement à `ChatOpenAI` et `OpenAIEmbeddings` au lieu de dépendre d’anciens alias de variables d’environnement.
  </Accordion>
