...
description: "列出透過 AI Sonar 建立的已完成 World Labs Marble 世界"
---

列出透過 AI Sonar 建立的已完成 World Labs Marble 世界。結果依你的組織隔離；AI Sonar 不會暴露共享上游帳號的完整列表。

待處理或失敗的生成任務請使用 `/v1/worlds/generations/{id}`。此列表只返回生成完成後已由 AI Sonar 索引的世界。
...
<ParamField body="status" type="string">
  可選狀態篩選。AI Sonar 對 `SUCCEEDED` 返回已完成 worlds；其他狀態屬於生成任務端點。
</ParamField>
