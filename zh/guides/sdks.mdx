---
title: "SDK 与库"
description: "用于 AI Sonar 集成的推荐 SDK 和库"
---

## 概述

AI Sonar 最容易按所需行为选择 SDK：
...
| 已有 OpenAI 兼容客户端 | OpenAI SDK + AI Sonar `/v1` |
...
<AccordionGroup>
  ...
  <Accordion title="迁移时使用 OpenAI 兼容 `/v1`">
    OpenAI 兼容 SDK 最适合已有 OpenAI 风格客户端、可移植聊天和 embeddings。它是兼容入口，不是 AI Sonar 唯一的 API 表面。
  </Accordion>

  <Accordion title="明确指定 base URL">
    在 client 配置中直接传入 AI Sonar 的 base URL，而不是依赖较旧的环境变量别名。
  </Accordion>
</AccordionGroup>
