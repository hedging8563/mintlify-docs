...
当前公共示例使用 `suno_music` 进行音乐生成，并在 `mv` 中传入 `chirp-v4` 等官方 Suno 模型版本。对于仅歌词的流程，发送 `action: "LYRICS"` 与公开契约文档中包含歌词生成的模型，并省略 `mv`。将模型 ID 视为公共 AI Sonar ID，而不是保证供应商特定字段是公共契约字段。
